 ...
 */
type Reverse_Bounce8<StateT extends Reverse_State> = Reverse_Bounce1<
	StateT
> extends infer S0
		...
		? Reverse_Bounce1<Extract<S1, Reverse_State>> extends infer S2
			? Reverse_Bounce1<Extract<S2, Reverse_State>> extends infer S3
				? Reverse_Bounce1<Extract<S3, Reverse_State>> extends infer S4
					? Reverse_Bounce1<
							Extract<S4, Reverse_State>
					  > extends infer S5
						? Reverse_Bounce1<
								Extract<S5, Reverse_State>
						  > extends infer S6
							? Reverse_Bounce1<
									Extract<S6, Reverse_State>
							  > extends infer S7
								? S7
								: never
							: never
 ...
 * ```
 */
type PopFront<ArrT extends readonly any[]> = ((...arr: ArrT) => void) extends (
	head: any,
	...arr: infer ResultT
) => void
	? ResultT
 ...
 * ```
 */
type PushFront<ArrT extends readonly any[], ElementT> = ((
	element: ElementT,
	...arr: ArrT
) => void) extends (...arr: infer ResultT) => void
	? ResultT
	: never;

type MaxDepth = 8;
type Countdown = [0, 0, 1, 2, 3, 4, 5, 6, 7];
type DecrementMaxDepth<MaxDepthT extends number> = Countdown[MaxDepthT];
